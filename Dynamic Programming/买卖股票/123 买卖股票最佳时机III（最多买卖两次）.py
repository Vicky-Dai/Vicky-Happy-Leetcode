...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for i in range(1, len(prices)):
            #第一次持有股票，有两种方式得到：1. 昨天就持有 2. 今天买入
            dp[i][1] = max(dp[i-1][1], - prices[i]) 
            #第一次不持有股票，有两种方式得到：1. 昨天就不持有 2. 今天卖出
            dp[i][2] = max(dp[i-1][2], dp[i-1][1] + prices[i])
            # 第二次持有股票，有两种方式得到：1. 昨天就第二次持有 2. 保持第一次不持有的状态，今天买入（因为只能买卖两次，所以第一次不持有股票的状态，就是第二次持有股票的状态）
            dp[i][3] = max(dp[i-1][3], dp[i-1][2] - prices[i])
            # 第二次不持有股票， 有两种方式得到：1. 昨天就第二次不持有 2.保持第二次持有，今天卖出
