...
#循环n-1次，建立n-1条边
#从节点视角来看： 每选中一个节点加入树（visted), 更新剩余节点到生成树的最短距离（minDist
...
for _ in range(1, v+1):
    ...
    for j in range(1, v+1): #依次遍历点
        if visited[j] == False and minDist[j] < min_val: 
            cur = j 
            min_val = minDist[j] #循环比较 找到距离生成树最近的
    visited[cur] = True #!!!!!!!!!缩进注意（一次只有一个cur能visit） 完成第一步，选取并标记为true
    """ 第二步，最近节点加入生成树 并更新剩余节点到生成树的最短距离 """
